...
<?php
// application/views/admin/transaction_account_view.php

include_once APPPATH . "views/partials/header.php";

$is_super_admin = ($this->session->userdata('role') === 'admin');
$can_expenses_edit = $is_super_admin || has_permission('Expenses', 'can_edit') || has_permission('Matumizi', 'can_edit');
$can_expenses_delete = $is_super_admin || has_permission('Expenses', 'can_delete') || has_permission('Matumizi', 'can_delete');
...
// if (!isset($account)) {
//     $account = [
//         (object)['trans_id' => 1, 'account_name' => 'NMB Bank'],
//         (object)['trans_id' => 2, 'account_name' => 'CRDB Bank'],
//         (object)['trans_id' => 3, 'account_name' => 'Cash In Hand'],
//         (object)['trans_id' => 4, 'account_name' => 'M-Pesa Till'],
//     ];
// }
// --- END DUMMY DATA ---
?>
    ...
    <div class="p-4 sm:p-6 space-y-6">
        ...
        <div class="mb-6">
            <h2 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-gray-200">
             <?php echo $this->lang->line('expenses_registration'); ?>
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
			<?php echo $this->lang->line('expenses_registration_desc'); ?>
            </p>
        </div>
        ...
        <!-- Card: Registration Account Form -->
        <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-800 dark:border-gray-700">
            <div class="p-4 md:p-6">
                <h3 class="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-6">
                    <?php echo $this->lang->line('register_daily_expenses'); ?>
                </h3>
                <?php if ($can_expenses_edit): ?>
...
<div class="grid sm:grid-cols-12 gap-4 sm:gap-6">
                        ...
                        <div class="sm:col-span-8">
                            <label for="account_name" class="block text-sm font-medium mb-2 dark:text-gray-300">* <?php echo $this->lang->line('expenses_name'); ?>:</label>
                            <input type="text" id="account_name" name="ex_name" placeholder="<?php echo $this->lang->line('expenses_name_placeholder'); ?>" autocomplete="off" required
                                   class=" uppercase py-2.5 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-cyan-500 focus:ring-cyan-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:placeholder-gray-500 dark:focus:ring-gray-600" value="<?php echo set_value('ex_name'); ?>">
        ...
        <!-- Card: Account List Table -->
        <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-800 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200"><?php echo $this->lang->line('expenses_list'); ?></h2>
            </div>
            ...
            <div class="p-4" data-hs-datatable='{
                ...
                "language": { "zeroRecords": "<div class=\"py-10 px-5 flex flex-col justify-center items-center text-center\"><svg class=\"shrink-0 size-6 text-gray-500 dark:text-gray-400\" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"11\" cy=\"11\" r=\"8\"/><path d=\"m21 21-4.3-4.3\"/></svg><div class=\"max-w-sm mx-auto\"><p class=\"mt-2 text-sm text-gray-600 dark:text-gray-400\"><?php echo $this->lang->line('no_expenses_found'); ?></p></div></div>" }
            }'>
                <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
                    <div class="relative max-w-xs w-full">
                        <label for="account-table-search" class="sr-only"><?php echo $this->lang->line('search'); ?></label>
                        <input type="text" name="account-table-search" id="account-table-search" class="py-2 px-3 ps-9 block w-full border-gray-200 shadow-sm rounded-lg text-sm focus:z-10 focus:border-cyan-500 focus:ring-cyan-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:placeholder-gray-500 dark:focus:ring-gray-600" placeholder="<?php echo $this->lang->line('search_expenses'); ?>" data-hs-datatable-search="#account_table">
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                  <?php $no = 1; ?>
									<?php foreach ($exp as $exps): ?>
                                        <tr>
											<td class="uppercase px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?= $no++; ?></td>
                                            <td class="uppercase px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $exps->ex_name; ?></td>
                                            <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                                <?php if ($can_expenses_delete): ?>
                                                <a href="<?php echo base_url("admin/delete_expenses/{$exps->ex_id}"); ?>" onclick="return confirm('<?php echo $this->lang->line('confirm_delete_expense'); ?>');" class="inline-flex items-center gap-x-1.5 text-sm font-semibold rounded-lg border border-transparent text-red-600 hover:text-red-800 disabled:opacity-50 disabled:pointer-events-none dark:text-red-500 dark:hover:text-red-400">
        ...
        <!-- End Card: Account List Table -->

        <?php // No modals were present in the original code for this page's list ?>
