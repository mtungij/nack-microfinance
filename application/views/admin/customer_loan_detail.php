    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <?php if (!empty($customer->passport)): ?>
          <img class="w-20 h-20 rounded-full object-cover border-4 border-green-400" src="<?= base_url($customer->passport) ?>" alt="Photo">
        <?php else: ?>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            <?php if (!empty($customer_loans)):
              ...
              foreach ($customer_loans as $cl):
                ...
                $lb = ucfirst($s);
                if ($s == 'open') { $b = 'bg-blue-100 text-blue-800'; }
                elseif ($s == 'aproved') { $b = 'bg-yellow-100 text-yellow-800'; }
                elseif ($s == 'disbarsed') { $b = 'bg-indigo-100 text-indigo-800'; }
                elseif ($s == 'withdrawal') { $b = 'bg-green-100 text-green-800'; $lb = 'Active'; }
                elseif ($s == 'out') { $b = 'bg-red-100 text-red-800'; $lb = 'Expired'; }
                elseif ($s == 'done') { $b = 'bg-emerald-100 text-emerald-800'; $lb = 'Full Paid'; }
            ?>
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              ...
              <td class="px-4 py-3"><span class="px-2 py-0.5 rounded-full text-xs font-medium <?= $b ?>"><?= $lb ?></span></td>
              <td class="px-4 py-3">
